...
public abstract class ListNode implements Comparable<Object> {
   ...
   */
   public int compareTo(Object l) {
	 ...
	 int compared = 0;
	 try {
	   cur = ((Integer) item()).intValue();
	   compared = ((Integer) l).intValue();
	 } catch (InvalidNodeException e) {
	   System.err.println(e);
	 }						
	 if(cur < compared) {
	   return -1;
	 } else if(cur == compared) {
	   return 0;
	 } else {
	   return 1;
	 }
   ...
   *  
   *  This method introduces a new invariant that in our Lists is that for every ListNode x in a List l, 
   *  x.myList = l, UNLESS x is the sentinel. For any sentinel node x, x.myList = null.
   *  
