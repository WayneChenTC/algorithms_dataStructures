...
private DListNode2 listHead;
private DListNode2 iterateNode;
  ...
  RunIterator(RunLengthEncoding rle) {
    ...
    iterateNode = rle.head.next;
    listHead = rle.head;
  }

  /**
   *  hasNext() returns true if this iterator has more runs, which means the iterator hasn't
   *  reach back to the head yet. 
   *  If it returns false, then the next call to next() may throw an exception.
   *
   *  @return true if the iterator has more elements.
   */
  public boolean hasNext() {
    // Replace the following line with your solution.
    return iterateNode != listHead;
  }
  ...
  public int[] next() {
    ...
    // Replace the following line with your solution.
    int[] runInfo = {((run) iterateNode).getLength(),((run) iterateNode).getRed(),((run) iterateNode).getGreen(),((run) iterateNode).getBlue()};
